...
/**
 * 桥梁结构结构线形检测记录表（全站仪，JGLP05009a-1）JSON → Word 渲染。
 */
@Component
public class AlignmentTotalStationJsonSheetRenderer extends AbstractStandardJsonSheetRenderer {

    private static final String TITLE = "桥梁结构结构线形检测记录表";
    private static final String SHEET_NO = "JGLP05009a-1";

    public AlignmentTotalStationJsonSheetRenderer() {
        super("alignment_total_station",
                "桥梁结构结构线形检测记录表（全站仪）.docx",
                "桥梁结构结构线形检测记录表（全站仪）",
                TITLE,
                SHEET_NO,
                Arrays.asList("测点", "坐标", "高程", "线形"),
                Arrays.asList("测点", "坐标", "高程"),
                Arrays.asList("station", "point", "x", "y", "z", "elevation", "designValue", "deviation", "note"));
    }
    ...
    private void fillCoordinateTable(XWPFDocument document, JSONObject pageJson, JSONObject header) {
        ...
        fillVerticalCoordinate(table, "校核点坐标", header, "checkPoint", findCoordinateRecord(records, "校核点坐标"));
    ...
    private void fillPointRow(XWPFTableRow row, JSONObject record) {
        ...
        if (cells.size() > 4) setCellText(cells.get(4), record.getString("elevation"));
        if (cells.size() > 5) setCellText(cells.get(5), record.getString("deviation"));
    }
